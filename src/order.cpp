...
static RtrBestResult decideWhereToRepair(DROID *psDroid);
...
void orderDroidBase(DROID *psDroid, DROID_ORDER_DATA *psOrder)
{
	...
	case DORDER_DROIDREPAIR:
		...
		debug(LOG_INFO, "DORDER_DROIDREPAIR another fella");
		break;
	case DORDER_OBSERVE:
	...
	case DORDER_RTR_SPECIFIED:
		{
			...
			else if (rtrData.type == RTR_TYPE_DROID && !isTransporter(psDroid))
			{
				...
				actionDroid(psDroid, DACTION_MOVETOREPAIRPOINT, psDroid->order.pos.x, psDroid->order.pos.y);
			}
			else
			{
	...
	if (secondaryCheckDamageLevelDeselect(psDroid, secondaryGetState(psDroid, DSO_REPAIR_LEVEL)))
	{
		    ...
		    !vtolRearming(psDroid))
		{
			...
			else
			{
				...
				{
					// DORDER_RTR_SPECIFIED allows units to open fire while DACTION_WAITFORREPAIR
					// Note that droids will shott stuff while waiting only, not DACTION_WAITDURINGREPAIR
					// this prevents from giving new orders to droids while they are being repaired...
					// plus, droids never stop approaching the facility (unlike when given direct specified order)
...
static inline RtrBestResult decideWhereToRepairAndBalance(DROID *psDroid)
{
	...
	for (STRUCTURE *psStruct = apsStructLists[psDroid->player]; psStruct; psStruct = psStruct->psNext)
	{
		...
		if (psStruct->pStructureType->type == REF_REPAIR_FACILITY && psStruct->status == SS_BUILT)
		{
			...
			vFacilityDist.push_back(thisDistToRepair);
	...
	for (DROID *psCurr = psdroidList; psCurr != nullptr; psCurr = psCurr->psNext)
	{
		if (psCurr->droidType == DROID_REPAIR || psCurr->droidType == DROID_CYBORG_REPAIR)
		{
			...
			vDroidDist.push_back(thisDistToRepair);
	...
	debug(LOG_INFO, "found a total of %lu RT, and %lu RF", vDroidDist.size(), vFacilityDist.size());

	// the center of this area starts at the closest repair droid/facility! NOT at the damaged droid!
	#define MAGIC_SUITABLE_REPAIR_AREA ((REPAIR_RANGE * 8)*(REPAIR_RANGE * 8))
	int bestDistToRepair = bestDistToRepairFac < bestDistToRepairDroid ? bestDistToRepairFac : bestDistToRepairDroid;

	// find all close enough repairing candidates
	for (int i=0; i < vFacilityDist.size(); i++)
	{
		if (abs(vFacilityDist[i] - bestDistToRepair) < MAGIC_SUITABLE_REPAIR_AREA)
		{
	...
	for (int i=0; i < vDroidDist.size(); i++)
	{
		if (abs(vDroidDist[i] - bestDistToRepair) < MAGIC_SUITABLE_REPAIR_AREA)
		{
	...
	} else if (vFacilityCloseEnough.size() > 1)
	{
		...
		debug(LOG_INFO, "going to repair facility %u", which);
...
static inline RtrBestResult decideWhereToRepair(DROID *psDroid)
{
	int iRepairFacDistSq = 0;
	DROID *psNext = nullptr;
	DROID *pSRepairDroid = nullptr;
	STRUCTURE *psHq = nullptr;
	STRUCTURE *psRepairFac = nullptr;
	RtrBestResult out;
	for (STRUCTURE *psStruct = apsStructLists[psDroid->player]; psStruct; psStruct = psStruct->psNext)
	{
	...
	if (!isTransporter(psDroid))
	{
		...
		{
			out.type = RTR_TYPE_DROID;
			out.psObj = pSRepairDroid;
			return out;
		}
	}
	if (psRepairFac != nullptr)
	{
		out.type = RTR_TYPE_REPAIR_FACILITY;
		out.psObj = psRepairFac;
		return out;
	}
	if (psHq != nullptr)
	{
		out.type = RTR_TYPE_HQ;
		out.psObj = psHq;
	}
	out.type = RTR_TYPE_NO_RESULT;
	return out;
}
